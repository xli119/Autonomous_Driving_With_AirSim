...
MODEL_OUTPUT_DIR = 'D:/EndToEndLearningRawData/model_output/drop=0.5_bright=0.4_shadow_300.30.9'
...
# We choose some images randomly and make them darker
random_dark = np.random.uniform(low=0.3, high=0.9)
for i in range(num_train_examples):
    p=0.3
    r = random.random()
    if r<p:
        train_dataset['image'][i] = cv2.cvtColor(train_dataset['image'][i], cv2.COLOR_RGB2HSV)
        train_dataset['image'][i][:, :, 2] = np.clip(train_dataset['image'][i][:, :, 2] * random_dark, 0, 255)
        train_dataset['image'][i] = cv2.cvtColor(train_dataset['image'][i], cv2.COLOR_HSV2RGB)


data_generator = DriveDataGenerator(rescale=1./255., horizontal_flip=True, brighten_range=0.4)
train_generator = data_generator.flow\
    (train_dataset['image'], train_dataset['previous_state'], train_dataset['label'], batch_size=batch_size, zero_drop_percentage=0.5, roi=[76,135,0,255])
eval_generator = data_generator.flow\
    (eval_dataset['image'], eval_dataset['previous_state'], eval_dataset['label'], batch_size=batch_size, zero_drop_percentage=0.5, roi=[76,135,0,255])
